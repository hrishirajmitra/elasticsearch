"""
SelfIndex with Library Compression (z=2)
Implements a custom inverted index with off-the-shelf compression library (zlib)
Plot.AB: Compression method comparison - Library implementation
"""
import json
import pickle
import zlib
...
class LibraryCompression:
    """Compression using standard zlib library"""
    
    @staticmethod
    def compress_list(numbers: List[int]) -> bytes:
        """Compress a list of integers using zlib"""
        # Convert list to bytes (4 bytes per int)
        data = struct.pack(f'{len(numbers)}I', *numbers)
        # Compress using zlib (level 6 = default, good balance)
        return zlib.compress(data, level=6)
    
    @staticmethod
    def decompress_list(data: bytes) -> List[int]:
        """Decompress a list of integers"""
        decompressed = zlib.decompress(data)
        count = len(decompressed) // 4
        return list(struct.unpack(f'{count}I', decompressed))
    
    @staticmethod
    def compress_list_with_delta(numbers: List[int]) -> bytes:
        """Compress using delta encoding + zlib for better compression"""
        if not numbers:
            return b''
    ...
    def decompress_list_with_delta(data: bytes) -> List[int]:
        ...
        decompressed = zlib.decompress(data)
        count = len(decompressed) // 4
        deltas = list(struct.unpack(f'{count}I', decompressed))
...
class SelfIndexZ2(IndexBase):
    """Self-implemented inverted index with library compression (zlib)"""
    ...
    def __init__(self):
        super().__init__(
            ...
            dstore='CUSTOM',
            qproc='TERMatat',
            compr='CLIB',  # Library compression
            optim='Null'
        )
        
        self.index_dir = Path("indices/selfindex_z2")
    ...
    def create_index(self, index_id: str = None, files: Iterable[Tuple[str, str]] = None) -> None:
        """Create inverted index with library compression"""
        ...
        print("Compressing postings lists with zlib...")
        for term, posting_list in term_postings.items():
            ...
            # Compress using zlib with delta encoding
            compressed_data = LibraryCompression.compress_list_with_delta(posting_list)
        ...
        # Save index to disk
        self._save_index(index_id or 'selfindex-z2-v1.0')
    ...
    def load_index(self, serialized_index_dump: str = None) -> None:
        ...
        if serialized_index_dump is None:
            serialized_index_dump = 'selfindex-z2-v1.0'
    ...
    def query(self, query_text: str, size: int = 10) -> str:
        ...
        for term in query_terms:
            ...
            doc_indices = LibraryCompression.decompress_list_with_delta(self.inverted_index[term])
    ...
    def get_stats(self) -> Dict:
        ...
        return {
            ...
            'compression_method': 'zlib (Delta + DEFLATE)',
            'doc_count': self.doc_count,
...
def main():
    ...
    # Create index
    indexer.create_index(index_id='selfindex-z2-v1.0')
